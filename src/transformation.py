...
from pyspark.sql.functions import abs, coalesce, col, create_map, initcap, lit, regexp_extract, trim, when
...
def drop_rows_with_missing_values(
    df,
    column: str,
    possible_missing_values: list[str],
):
    ...
    if removed_count > 0:
        logger.info(
            "Removed %s rows with missing values in column %s.",
            removed_count,
            column,
        )
...
def normalize_categorical_text_columns(
    ...
    if possible_missing_values is None:
        possible_missing_values = ["", "Na", "N/a", "Null", "Nan"]

    for column_name in columns:
        missing_condition = col(column_name).isNull() | trim(col(column_name)).isin(
            possible_missing_values
        )

        df = df.withColumn(
            column_name,
            when(missing_condition, lit(None)).otherwise(initcap(trim(col(column_name)))),
        )

    logger.info(
        "Normalized %s categorical text columns: %s",
        len(columns),
        columns,
    )
...
def normalize_price_currency_code(
    ...
    missing_condition = col(price_column).isNull() | trim(col(price_column)).isin(
        possible_missing_values
    )

    df = df.withColumn(
        price_column,
        when(missing_condition, lit(None)).otherwise(upper(trim(col(price_column)))),
    )

    logger.info(
        "Normalized non-missing %s values and preserved missing values as null.",
        price_column,
    )
...
def parse_price_column(
    ...
    df = df.withColumn(
        ...
        when(
            ...
            lit(False),
        ).otherwise(price_text.rlike(price_pattern)),
    )
    ...
    df = df.withColumn(
        ...
        when(
            col(is_valid_column),
            regexp_extract(price_text, price_pattern, 1),
        ),
    )
    ...
    df = df.withColumn(
        ...
        when(
            col(is_valid_column),
            regexp_extract(price_text, price_pattern, 2).cast(DoubleType()),
        ),
    )

    logger.info(
        "Parsed %s into %s and %s; added %s validity flag.",
        price_column,
        currency_column,
        amount_column,
        is_valid_column,
...
def convert_negative_days_to_positive(df, day_column: str):
    ...
    logger.info(
        "Cleaned negative values in column %s by taking absolute values.",
        day_column,
    )
...
def convert_num_adults_words_to_digits(df, num_adults_column: str):
    ...
    df = df.withColumn(
        num_adults_column,
        coalesce(mapping_expr[trim(col(num_adults_column))], col(num_adults_column)),
    )
...
def transform_noshow_data(
    ...
    columns_to_check = [
        field.name for field in df.schema.fields if field.name not in ["room", "price"]
    ]
    ...
    for column_name in columns_to_check:
        df = drop_rows_with_missing_values(
            df,
            column=column_name,
            possible_missing_values=possible_missing_values,
        )
    ...
    df = normalize_categorical_text_columns(
        df,
        categorical_string_columns,
        possible_missing_values=possible_missing_values,
    )
    df = normalize_price_currency_code(
        df,
        price_column="price",
        possible_missing_values=possible_missing_values,
    )
